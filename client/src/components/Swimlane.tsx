...
export default function Swimlane({ title, tickets, deleteTicket }: SwimlaneProps) {
  ...
  return (
    <section className="rounded-2xl bg-(--brand-panel)/60 ring-1 ring-[var(--brand-ring)] shadow-md">
      <div className="flex items-center justify-between px-4 py-3 text-sm font-semibold tracking-wide text-(--brand-muted) uppercase">
        <span>{title}</span>
      ...
      <div className="space-y-3 p-3">
        ...
        {!tickets?.length && (
          <div className="rounded-lg border border-dashed border-white/10 p-4 text-center text-sm text-(--brand-muted)">
            No tickets yet
          </div>
        )}
      </div>
